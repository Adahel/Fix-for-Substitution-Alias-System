...
public class GameData
{
    ...
    private static class BlockStateCapture implements FMLControlledNamespacedRegistry.AddCallback<Block>
    {
		...
		@Override
		public void onSubstituteActivated(Block original, Block replacement, ResourceLocation name)
		{
            if (BLOCK_TO_ITEM.containsKey(original))
            {
            	Item i = BLOCK_TO_ITEM.get(original);
                if (i instanceof ItemBlock)
                {
                    try
                    {
                    	EnumHelper.setFailsafeFieldValue(ReflectionHelper.findField(ItemBlock.class, "block", "field_150939" + "_a"), i, replacement);
                    }
                    catch (Exception e)
                    {
                    	FMLLog.log(Level.FATAL, e, "Cannot set the 'block' field from ItemBlock, this is fatal!");
                        throw Throwables.propagate(e);
                    }
                }
                BLOCK_TO_ITEM.forcePut(replacement,i);
            }
		}
    }
    ...
    private static class ItemBlockCapture implements FMLControlledNamespacedRegistry.AddCallback<Item>
    {
        ...
        public void onAdd(Item item, int blockId)
        {
            if (item instanceof ItemBlock)
            {
                ...
                BLOCK_TO_ITEM.forcePut(block, item);
            }
        }

		@Override
		public void onSubstituteActivated(Item original, Item replacement, ResourceLocation name) {}
    }
    ...
    private static class PotionArrayCapture implements FMLControlledNamespacedRegistry.AddCallback<Potion>
    {
		...
		@Override
		public void onSubstituteActivated(Potion original, Potion replacement, ResourceLocation name) {}
    }
}